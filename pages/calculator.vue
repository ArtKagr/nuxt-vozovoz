...
<script>
import Address from "../components/calculator/Address";
import Terminal from "../components/calculator/Terminal";
import CityPoint from "../components/calculator/CityPoint";
export default {
  head() {
    return {
      title: 'Калькулятор',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: 'Калькулятор расчёта доставки'
        }
      ]
    }
  },
  ...
  computed: {
    departurePoint() {
      return this.$store.getters['calculator/getDepartureOption']
    },
    receivingPoint() {
      return this.$store.getters['calculator/getReceivingOption']
    },
    priceFilter() {
      return this.$store.getters['calculator/getPriceFilter']
    },
    priceCalculate() {
      return this.$store.getters['calculator/getPriceCalculate']
    }
  },
  methods: {
    fetchPrice() {
    ...
    setWeight() {
      ...
      this.$store.commit('calculator/UPDATE_WEIGHT', this.weight)
    },
    setValue() {
      if(this.volume > 74) this.volume = 74
      else if(this.volume < 0.01) this.volume = 0.01
      this.$store.commit('calculator/UPDATE_VOLUME', this.volume)
    }
  },
  created() {
    this.$store.dispatch('calculator/fetchLocations')
    this.$store.dispatch('calculator/fetchPrice')
  }
}
</script>
