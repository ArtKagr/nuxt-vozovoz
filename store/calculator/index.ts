...
export const getters: GetterTree<calculatorState, calculatorState> = {
...
export const mutations: MutationTree<calculatorState> = {
  SET_STATUS(state: calculatorState, status: string) {
    state.status = status
  },
  UPDATE_DEPARTURE_OPTION(state: calculatorState, option: any) {
    state.departureOption = option
    state.priceFilter.from.type = option
  },
  UPDATE_RECEIVING_OPTION(state: calculatorState, option: any) {
    state.receivingOption = option
    state.priceFilter.to.type = option
  },
  SAVE_PRICE_CALCULATE(state: calculatorState, priceCalculate: any) {
    state.priceCalculate = priceCalculate
  },
  SAVE_LOCATIONS(state: calculatorState, locations: any) {
    state.locations = locations
  },
  UPDATE_PRICE_LOCATION(state: calculatorState, query: any) {
    if(query.source === 'departure') state.priceFilter.from.guid = query.item
    else state.priceFilter.to.guid = query.item
  },
  UPDATE_WEIGHT(state: calculatorState, weight: any) {
    state.priceFilter.weight = weight
  },
  UPDATE_VOLUME(state: calculatorState, volume: any) {
    state.priceFilter.volume = volume
  }
}

export const actions: ActionTree<calculatorState, calculatorState> = {
