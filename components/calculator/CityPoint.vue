...
export default {
  ...
  computed: {
    pointOption() {
      if(this.source === 'departure') return this.$store.getters['calculator/getDepartureOption']
      else return this.$store.getters['calculator/getReceivingOption']
    }
  },
  methods: {
    setDeparturePoint(point) {
      if(this.source === 'departure') this.$store.commit('calculator/UPDATE_DEPARTURE_OPTION', point)
      else this.$store.commit('calculator/UPDATE_RECEIVING_OPTION', point)
    }
  }
}
</script>
