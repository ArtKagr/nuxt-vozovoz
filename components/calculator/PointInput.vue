...
export default {
  ...
  computed: {
    ...
    items() {
      if(this.point) return this.$store.getters['calculator/getAddresses']
      else return this.$store.getters['calculator/getLocations']
    },
  },
  methods: {
    showItemsList() {
    ...
    setItem(item) {
      this.$store.commit('calculator/UPDATE_PRICE_LOCATION', { item: item.guid, source: this.source })
  ...
  created() {
    if(this.point) this.itemsCopy = this.$store.getters['calculator/getAddresses']
    else this.itemsCopy = this.$store.getters['calculator/getLocations']
